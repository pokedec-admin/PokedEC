...
@Component({
  selector: 'app-dynamax',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="dynamax-container">
      <h2>🌀 Mes Pokémon Dynamax</h2>
      <p class="subtitle">Pokémon Dynamax collectés</p>
      ...
      <div class="regions-container">
        <div *ngFor="let region of regions" class="region-section">
          ...
          <p *ngIf="region.pokemon.length === 0" class="empty-region">Aucun Pokémon Dynamax de {{ region.name }} pour le moment.</p>
        </div>
      </div>

      <div *ngIf="myPokemon.length === 0" class="no-pokemon">
        <p>Aucun Pokémon Dynamax pour le moment.</p>
        <p><small>Allez dans le Pokédex et cochez la forme "Dynamax" !</small></p>
      </div>
    </div>
  `,
  styles: [`
    .dynamax-container { padding: 20px; max-width: 1200px; margin: 0 auto; }
    ...
    .region-header:hover { color: #2980b9; background: #f7fbfe; }
    ...
    .pokemon-table th { background-color: #3498db; color: white; padding: 12px; text-align: left; font-weight: bold; }
    .pokemon-table td { padding: 12px; border-bottom: 1px solid #eee; }
    .clickable-row { cursor: pointer; transition: background 0.2s; }
    .clickable-row:hover { background-color: #ebf5fb; }
...
export class Dynamax implements OnInit {
  ...
  ngOnInit() {
    this.pokemonService.getMyPokedex().subscribe({
      next: (data: PokemonEntry[]) => {
        console.log('Total Pokedex data:', data.length);
        const dynamax = data.filter(p => p.form_name === 'Dynamax');
        console.log('Dynamax forms found:', dynamax.length);
        this.myPokemon = dynamax.filter(p => p.has_normal || p.has_dynamax);
        console.log('Collected Dynamax:', this.myPokemon.length);
        this.groupByRegion();
      },
      error: (err: any) => console.error('Failed to load Dynamax pokemon', err)
    });
  }
