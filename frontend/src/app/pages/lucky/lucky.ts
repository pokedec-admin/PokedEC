...
@Component({
  ...
  template: `
    <div class="lucky-container">
      <h2>🍀 Mes Pokémon Chanceux</h2>
      <p class="subtitle">Pokémon Chanceux collectés</p>
      ...
      <div class="regions-container">
        <div *ngFor="let region of regions" class="region-section">
          ...
          <p *ngIf="region.pokemon.length === 0" class="empty-region">Aucun Pokémon Chanceux de {{ region.name }} pour le moment.</p>
        </div>
      </div>

      <div *ngIf="myPokemon.length === 0" class="no-pokemon">
        <p>Aucun Pokémon Chanceux pour le moment.</p>
        <p><small>Allez dans le Pokédex et cochez la case "Chanceux" !</small></p>
      </div>
    </div>
  `,
  styles: [`
    .lucky-container { padding: 20px; max-width: 1200px; margin: 0 auto; }
    ...
    .region-header:hover { color: #43a047; background: #f1f8f1; }
    ...
    .pokemon-table th { background-color: #4caf50; color: white; padding: 12px; text-align: left; font-weight: bold; }
...
export class Lucky implements OnInit {
  ...
  ngOnInit() {
    this.pokemonService.getMyPokedex().subscribe({
      next: (data: PokemonEntry[]) => {
        this.myPokemon = data.filter(p => p.has_lucky);
        this.groupByRegion();
      },
      error: (err: any) => console.error('Failed to load Lucky pokemon', err)
    });
  }
