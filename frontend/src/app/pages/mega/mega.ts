...
@Component({
  ...
  template: `
    <div class="mega-container">
      <h2>🧬 Mes Pokémon Méga</h2>
      <p class="subtitle">Pokémon Méga collectés</p>
      ...
      <div class="regions-container">
        <div *ngFor="let region of regions" class="region-section">
          ...
          <p *ngIf="region.pokemon.length === 0" class="empty-region">Aucun Pokémon Méga de {{ region.name }} pour le moment.</p>
        </div>
      </div>

      <div *ngIf="myPokemon.length === 0" class="no-pokemon">
        <p>Aucun Pokémon Méga pour le moment.</p>
        <p><small>Allez dans le Pokédex et cochez la forme "Méga" !</small></p>
      </div>
    </div>
  `,
  styles: [`
    .mega-container { padding: 20px; max-width: 1200px; margin: 0 auto; }
    ...
    .region-header:hover { color: #d35400; background: #fff5eb; }
    ...
    .pokemon-table th { background-color: #e67e22; color: white; padding: 12px; text-align: left; font-weight: bold; }
  ...
  ngOnInit() {
    this.pokemonService.getMyPokedex().subscribe({
      next: (data: PokemonEntry[]) => {
        this.myPokemon = data.filter(p => p.form_name.startsWith('Méga') && p.has_normal);
        this.groupByRegion();
      },
      error: (err: any) => console.error('Failed to load Mega pokemon', err)
    });
  }
