...
@Component({
  selector: 'app-obscure',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="obscure-container">
      <h2>👿 Mes Pokémon Obscurs</h2>
      <p class="subtitle">Pokémon Obscurs collectés</p>
      ...
      <div class="regions-container">
        <div *ngFor="let region of regions" class="region-section">
          ...
          <p *ngIf="region.pokemon.length === 0" class="empty-region">Aucun Pokémon Obscur de {{ region.name }} pour le moment.</p>
        </div>
      </div>

      <div *ngIf="myPokemon.length === 0" class="no-pokemon">
        <p>Aucun Pokémon Obscur pour le moment.</p>
        <p><small>Allez dans le Pokédex et cochez la case "Obscur" !</small></p>
      </div>
    </div>
  `,
  styles: [`
    .obscure-container { padding: 20px; max-width: 1200px; margin: 0 auto; }
    ...
    .region-header:hover { color: #7d3c98; background: #f5eef8; }
    ...
    .pokemon-table th { background-color: #8e44ad; color: white; padding: 12px; text-align: left; font-weight: bold; }
    .pokemon-table td { padding: 12px; border-bottom: 1px solid #eee; }
    .clickable-row { cursor: pointer; transition: background 0.2s; }
    .clickable-row:hover { background-color: #f4ecf7; }
...
export class Obscure implements OnInit {
  ...
  ngOnInit() {
    this.pokemonService.getMyPokedex().subscribe({
      next: (data: PokemonEntry[]) => {
        this.myPokemon = data.filter(p => p.has_obscure);
        this.groupByRegion();
      },
      error: (err: any) => console.error('Failed to load Obscure pokemon', err)
    });
  }
