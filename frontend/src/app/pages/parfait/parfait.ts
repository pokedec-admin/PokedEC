...
@Component({
  selector: 'app-parfait',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="parfait-container">
      <h2>💯 Mes Pokémon Parfaits (100%)</h2>
      <p class="subtitle">Pokémon avec des stats parfaites collectés</p>
      ...
      <div class="regions-container">
        <div *ngFor="let region of regions" class="region-section">
          ...
          <p *ngIf="region.pokemon.length === 0" class="empty-region">Aucun Pokémon Parfait de {{ region.name }} pour le moment.</p>
        </div>
      </div>

      <div *ngIf="myPokemon.length === 0" class="no-pokemon">
        <p>Aucun Pokémon Parfait pour le moment.</p>
        <p><small>Allez dans le Pokédex et cochez la case "Parfait" !</small></p>
      </div>
    </div>
  `,
  styles: [`
    .parfait-container { padding: 20px; max-width: 1200px; margin: 0 auto; }
    ...
    .region-header:hover { color: #c0392b; background: #fdf2f2; }
    ...
    .pokemon-table th { background-color: #e74c3c; color: white; padding: 12px; text-align: left; font-weight: bold; }
    .pokemon-table td { padding: 12px; border-bottom: 1px solid #eee; }
    .clickable-row { cursor: pointer; transition: background 0.2s; }
    .clickable-row:hover { background-color: #fceae9; }
...
export class Parfait implements OnInit {
  ...
  ngOnInit() {
    this.pokemonService.getMyPokedex().subscribe({
      next: (data: PokemonEntry[]) => {
        this.myPokemon = data.filter(p => p.has_parfait);
        this.groupByRegion();
      },
      error: (err: any) => console.error('Failed to load Parfait pokemon', err)
    });
  }
