...
@Component({
  selector: 'app-purifie',
  standalone: true,
  imports: [CommonModule],
  template: `
    <div class="purifie-container">
      <h2>🌟 Mes Pokémon Purifiés</h2>
      <p class="subtitle">Pokémon Purifiés collectés</p>
      ...
      <div class="regions-container">
        <div *ngFor="let region of regions" class="region-section">
          ...
          <p *ngIf="region.pokemon.length === 0" class="empty-region">Aucun Pokémon Purifié de {{ region.name }} pour le moment.</p>
        </div>
      </div>

      <div *ngIf="myPokemon.length === 0" class="no-pokemon">
        <p>Aucun Pokémon Purifié pour le moment.</p>
        <p><small>Allez dans le Pokédex et cochez la case "Purifié" !</small></p>
      </div>
    </div>
  `,
  styles: [`
    .purifie-container { padding: 20px; max-width: 1200px; margin: 0 auto; }
    ...
    .region-header:hover { color: #219150; background: #f0f9f4; }
...
export class Purifie implements OnInit {
  ...
  ngOnInit() {
    this.pokemonService.getMyPokedex().subscribe({
      next: (data: PokemonEntry[]) => {
        this.myPokemon = data.filter(p => p.has_purifie);
        this.groupByRegion();
      },
      error: (err: any) => console.error('Failed to load Purifie pokemon', err)
    });
  }
