...
@Component({
    ...
    <div class="regional-container">
      <h2>📍 Mes Pokémon Régionaux</h2>
      <p class="subtitle">Pokémon exclusifs géographiquement collectés</p>
      ...
      <div class="regions-container">
        <div *ngFor="let region of regions" class="region-section">
                ...
                <tr *ngFor="let poke of region.pokemon" (click)="goToDetail(poke.pokemon_id)" class="clickable-row">
                  ...
                  <td class="location-cell">
                    <span class="location-badge">
                      ...
                      {{ poke.regional_description || 'Zone non spécifiée' }}
                    </span>
                  </td>
                </tr>
              </tbody>
          ...
          <p *ngIf="region.pokemon.length === 0" class="empty-region">Aucun Pokémon Régional de {{ region.name }} pour le moment.</p>
        </div>
      </div>

      <div *ngIf="myPokemon.length === 0" class="no-pokemon">
        <p>Aucun Pokémon Régional pour le moment.</p>
        <p><small>Indiquez quels Pokémon sont régionaux dans l'admin !</small></p>
      </div>
    </div>
  `,
    styles: [`
    .regional-container { padding: 20px; max-width: 1200px; margin: 0 auto; }
    ...
    .region-header:hover { color: #e67e22; background: #fffaf0; }
    ...
    .pokemon-table th { background-color: #d35400; color: white; padding: 12px; text-align: left; font-weight: bold; }
    .pokemon-table td { padding: 12px; border-bottom: 1px solid #eee; vertical-align: middle; }
    .clickable-row { cursor: pointer; transition: background 0.2s; }
    .clickable-row:hover { background-color: #fdf2e9; }
...
export class Regional implements OnInit {
    ...
    ngOnInit() {
        this.pokemonService.getMyPokedex().subscribe({
            next: (data: PokemonEntry[]) => {
                // For this page, we show pokemon that are marked as regional AND the user HAS the normal version
                // Or maybe the user wants to see all regional pokemon? 
                // Usually these category pages show what the user POSSESSES.
                this.myPokemon = data.filter(p => p.is_regional && p.has_normal);
                this.groupByRegion();
            },
            error: (err: any) => console.error('Failed to load Regional pokemon', err)
        });
    }
