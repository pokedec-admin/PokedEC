...
@Component({
  ...
  template: `
    <div class="xxl-container">
      <h2>📏 Mes Pokémon XXL</h2>
      <p class="subtitle">Pokémon de taille XXL collectés</p>
      ...
      <div class="regions-container">
        <div *ngFor="let region of regions" class="region-section">
          ...
          <p *ngIf="region.pokemon.length === 0" class="empty-region">Aucun Pokémon XXL de {{ region.name }} pour le moment.</p>
        </div>
      </div>

      <div *ngIf="myPokemon.length === 0" class="no-pokemon">
        <p>Aucun Pokémon XXL pour le moment.</p>
        <p><small>Allez dans le Pokédex et cochez la case "XXL" !</small></p>
      </div>
    </div>
  `,
  styles: [`
    .xxl-container { padding: 20px; max-width: 1200px; margin: 0 auto; }
    ...
    .region-header:hover { color: #2c3e50; background: #f4f6f7; }
    ...
    .pokemon-table th { background-color: #34495e; color: white; padding: 12px; text-align: left; font-weight: bold; }
    .pokemon-table td { padding: 12px; border-bottom: 1px solid #eee; }
    .clickable-row { cursor: pointer; transition: background 0.2s; }
    .clickable-row:hover { background-color: #f2f4f4; }
...
export class XXL implements OnInit {
  ...
  ngOnInit() {
    this.pokemonService.getMyPokedex().subscribe({
      next: (data: PokemonEntry[]) => {
        this.myPokemon = data.filter(p => p.has_xxl);
        this.groupByRegion();
      },
      error: (err: any) => console.error('Failed to load XXL pokemon', err)
    });
  }
