...
@Component({
  ...
  template: `
    <div class="xxs-container">
      <h2>🤏 Mes Pokémon XXS</h2>
      <p class="subtitle">Pokémon de taille XXS collectés</p>
      ...
      <div class="regions-container">
        <div *ngFor="let region of regions" class="region-section">
          ...
          <p *ngIf="region.pokemon.length === 0" class="empty-region">Aucun Pokémon XXS de {{ region.name }} pour le moment.</p>
        </div>
      </div>

      <div *ngIf="myPokemon.length === 0" class="no-pokemon">
        <p>Aucun Pokémon XXS pour le moment.</p>
        <p><small>Allez dans le Pokédex et cochez la case "XXS" !</small></p>
      </div>
    </div>
  `,
  styles: [`
    .xxs-container { padding: 20px; max-width: 1200px; margin: 0 auto; }
    ...
    .region-header:hover { color: #5d6d7e; background: #f8f9f9; }
    ...
    .pokemon-table th { background-color: #7f8c8d; color: white; padding: 12px; text-align: left; font-weight: bold; }
    .pokemon-table td { padding: 12px; border-bottom: 1px solid #eee; }
    .clickable-row { cursor: pointer; transition: background 0.2s; }
    .clickable-row:hover { background-color: #f2f3f4; }
...
export class XXS implements OnInit {
  ...
  ngOnInit() {
    this.pokemonService.getMyPokedex().subscribe({
      next: (data: PokemonEntry[]) => {
        this.myPokemon = data.filter(p => p.has_xxs);
        this.groupByRegion();
      },
      error: (err: any) => console.error('Failed to load XXS pokemon', err)
    });
  }
